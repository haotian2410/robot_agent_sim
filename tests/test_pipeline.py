...
def test_pick_and_place_route_a():
    r=PipelineEngine().plan("把左边的红色方块放进右边蓝色盒子",seed=7); assert r.status=="accepted" and r.model_call_count==2; assert [x["skill_name"] for x in r.skill_plan["steps"]]==["locate","move","grasp","locate","move","release"]
...
def test_iou_and_ambiguity():
    assert iou((0,0,100,100),(0,0,100,100))==1; d=VisionDetection(entity_id="x",bbox=[0,0,100,100],confidence=.9); m,u,a=match_detections([d],[{"object_id":"a","bbox":[0,0,100,100]},{"object_id":"b","bbox":[0,0,100,100]}]); assert not m and a==["x"]
